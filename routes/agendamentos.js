...
app.get('/:id_cliente', async (req, res) => {
  const select = 'SELECT * from agendamentos where id_cliente = ? ';
  try {
    const [results] = await bd.query(select, [req.params.id_cliente]);
    res.send(results);
  } catch (err) {
    console.log(err);
    res.status(500).send('Erro ao buscar dados da tabela (agendamento)!');
...
app.post('/insert', async (req, res) => {
  const insert =
    'INSERT INTO agendamentos (data_dia,id_cliente,descricao,status) VALUES (?,?,?,?)';
  const { data_dia, id_cliente, descricao, status } = req.body;
  try {
...
app.put('/insert/:id', async (req, res) => {
  const update =
    'UPDATE agendamentos SET data_dia = ?, id_cliente = ?, descricao = ?, status = ? WHERE id=?';
  const { data_dia, id_cliente, descricao, status } = req.body;
  try {
